<div>
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div wire:click="openProductModal({{ $menu->id }})"
                    ...
                    <div class="p-6">
                        ...
                        <div class="flex items-center justify-between">
                            <span class="text-2xl font-bold text-amber-800">Rp{{ number_format($menu->harga, 0, ',', '.') }}</span>
                            @if ($menu->stock > 0)
            ...
            @empty
                <p class="col-span-full text-center text-gray-600 text-lg">Tidak ada item menu ditemukan yang sesuai dengan pencarian Anda atau di kategori ini.</p>
            @endforelse
        </div>
    </div>
    ...
    <div class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4"
        x-data="{ show: @entangle('showProductModal') }" x-show="show" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
        x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
        @click.away="show = false; $wire.closeProductModal()">
            ...
            <div class="flex flex-col md:flex-row gap-6">
                <div class="md:w-1/2 flex justify-center items-center">
                    @if($selectedMenu->gambar && file_exists(public_path('storage/' . $selectedMenu->gambar)))
                        <img src="{{ asset('storage/' . $selectedMenu->gambar) }}" alt="{{ $selectedMenu->nama }}" class="max-h-80 w-auto rounded-lg shadow-md object-cover">
                    @else
                        <img src="{{ asset('gambar/gambar-default.jpg') }}" alt="{{ $selectedMenu->nama }}" class="max-h-80 w-auto rounded-lg shadow-md object-cover">
                    @endif
                </div>
                <div class="md:w-1/2">
                    <h3 class="text-3xl font-bold text-gray-800 mb-2">{{ $selectedMenu->nama }}</h3>
                    <p class="text-2xl font-bold text-amber-800 mb-4">Rp{{ number_format($selectedMenu->harga, 0, ',', '.') }}</p>
                    ...
                    <div class="flex items-center space-x-4 mb-6">
                        ...
                        <div class="flex items-center border border-gray-300 rounded-md">
                            <button wire:click="decrementQuantity" class="px-3 py-1 text-lg font-bold text-amber-700 hover:bg-gray-100 rounded-l-md" {{ $modalQuantity <= 1 ? 'disabled' : '' }}>-</button>
                            <span class="px-4 py-1 text-lg font-semibold text-gray-800">{{ $modalQuantity }}</span>
                            <button wire:click="incrementQuantity" class="px-3 py-1 text-lg font-bold text-amber-700 hover:bg-gray-100 rounded-r-md" {{ $modalQuantity >= $selectedMenu->stock ? 'disabled' : '' }}>+</button>
                        </div>
                    </div>
                    @if ($selectedMenu->stock > 0)
                        <p class="text-sm font-semibold text-gray-600 mb-4">Stok Tersedia: {{ $selectedMenu->stock }}</p>
                    @else
                        <p class="text-sm font-semibold text-red-600 mb-4">Stok Habis!</p>
                    @endif
                    <button wire:click="addToCart({{ $selectedMenu->id }}, '{{ $selectedMenu->nama }}', {{ (float) $selectedMenu->harga }}, {{ $modalQuantity }})"
